...
resource "aws_iam_policy" "karpenter_nodegroup_passrole" {
  name   = "${var.cluster_name}-${var.stage}-karpenter-nodegroup-passrole"
  policy = data.aws_iam_policy_document.karpenter_nodegroup_passrole.json
...
data "aws_iam_policy_document" "karpenter_nodegroup_passrole" {
  statement {
    actions = ["iam:PassRole"]
    effect  = "Allow"
    resources = [
      module.eks.eks_managed_node_groups["frontend"].iam_role_arn,
      module.eks.eks_managed_node_groups["backend"].iam_role_arn,
    ]
  }
}
...
module "karpenter" {
  ...
  policies = {
    karpenter_nodegroup_passrole = aws_iam_policy.karpenter_nodegroup_passrole.arn
  }
